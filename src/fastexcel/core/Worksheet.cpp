...
// 列式存储模式实现 (委托给 ColumnarStorageManager)

void Worksheet::enableColumnarMode(const WorkbookOptions* options) {
    if (!columnar_storage_) {
        columnar_storage_ = std::make_unique<ColumnarStorageManager>();
        columnar_storage_->enableColumnarStorage(options);
        FASTEXCEL_LOG_INFO("启用列式存储模式 for worksheet: {}", name_);
    }
}

bool Worksheet::isColumnarMode() const {
    return columnar_storage_ && columnar_storage_->isColumnarEnabled();
}

void Worksheet::setColumnarValue(uint32_t row, uint32_t col, double value) {
    if (!columnar_storage_) {
        enableColumnarMode();
    }
    columnar_storage_->setValue(row, col, value);
}

void Worksheet::setColumnarValue(uint32_t row, uint32_t col, uint32_t sst_index) {
    if (!columnar_storage_) {
        enableColumnarMode();
    }
    columnar_storage_->setValue(row, col, sst_index);
}

void Worksheet::setColumnarValue(uint32_t row, uint32_t col, bool value) {
    if (!columnar_storage_) {
        enableColumnarMode();
    }
    columnar_storage_->setValue(row, col, value);
...
void Worksheet::setColumnarValue(uint32_t row, uint32_t col, const std::tm& datetime) {
    if (!columnar_storage_) {
        enableColumnarMode();
    }
    columnar_storage_->setValue(row, col, datetime);
}

void Worksheet::setColumnarFormula(uint32_t row, uint32_t col, uint32_t formula_index, double result) {
    if (!columnar_storage_) {
        enableColumnarMode();
    }
    columnar_storage_->setFormula(row, col, formula_index, result);
}

void Worksheet::setColumnarError(uint32_t row, uint32_t col, const std::string& error_code) {
    if (!columnar_storage_) {
        enableColumnarMode();
    }
    columnar_storage_->setError(row, col, error_code);
}

bool Worksheet::hasColumnarValue(uint32_t row, uint32_t col) const {
    return columnar_storage_ && columnar_storage_->hasValue(row, col);
}

Worksheet::ColumnarValueVariant Worksheet::getColumnarValue(uint32_t row, uint32_t col) const {
    if (!columnar_storage_) {
        return std::monostate{};
    }
    return columnar_storage_->getValue(row, col);
}

void Worksheet::forEachInColumn(uint32_t col, std::function<void(uint32_t row, const ColumnarValueVariant& value)> callback) const {
    if (columnar_storage_) {
        columnar_storage_->forEachInColumn(col, callback);
    }
}

std::unordered_map<uint32_t, double> Worksheet::getNumberColumn(uint32_t col) const {
    if (columnar_storage_) {
        return columnar_storage_->getNumberColumn(col);
    }
    return {};
}

std::unordered_map<uint32_t, uint32_t> Worksheet::getStringColumn(uint32_t col) const {
    if (columnar_storage_) {
        return columnar_storage_->getStringColumn(col);
    }
    return {};
}

std::unordered_map<uint32_t, bool> Worksheet::getBooleanColumn(uint32_t col) const {
    if (columnar_storage_) {
        return columnar_storage_->getBooleanColumn(col);
    }
    return {};
}

std::unordered_map<uint32_t, double> Worksheet::getDateTimeColumn(uint32_t col) const {
    if (columnar_storage_) {
        return columnar_storage_->getDateTimeColumn(col);
    }
    return {};
}

std::unordered_map<uint32_t, ColumnarStorageManager::FormulaValue> Worksheet::getFormulaColumn(uint32_t col) const {
    if (columnar_storage_) {
        return columnar_storage_->getFormulaColumn(col);
    }
    return {};
}

std::unordered_map<uint32_t, std::string> Worksheet::getErrorColumn(uint32_t col) const {
    if (columnar_storage_) {
        return columnar_storage_->getErrorColumn(col);
    }
    return {};
}

size_t Worksheet::getColumnarDataCount() const {
    if (columnar_storage_) {
        return columnar_storage_->getDataCount();
    }
    return 0;
}

size_t Worksheet::getColumnarMemoryUsage() const {
    if (columnar_storage_) {
        return columnar_storage_->getMemoryUsage();
    }
    return 0;
}

void Worksheet::clearColumnarData() {
    if (columnar_storage_) {
        columnar_storage_->clearData();
        FASTEXCEL_LOG_INFO("清除列式存储数据 for worksheet: {}", name_);
    }
}
