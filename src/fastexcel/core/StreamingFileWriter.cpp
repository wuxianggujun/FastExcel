...
StreamingFileWriter::~StreamingFileWriter() {
    if (streaming_file_open_) {
        LOG_WARN("StreamingFileWriter destroyed with open streaming file: {}", current_streaming_path_);
...
bool StreamingFileWriter::writeFile(const std::string& path, const std::string& content) {
    if (streaming_file_open_) {
        LOG_ERROR("Cannot write file while streaming file is open: {}", current_streaming_path_);
        return false;
    }
    
    LOG_DEBUG("Writing file directly in streaming mode: {} ({} bytes)", path, content.size());
    ...
    if (success) {
        ...
        LOG_DEBUG("Successfully wrote file: {}", path);
    } else {
...
bool StreamingFileWriter::openStreamingFile(const std::string& path) {
    if (streaming_file_open_) {
        LOG_ERROR("Streaming file already open: {}", current_streaming_path_);
        return false;
    }
    ...
    if (success) {
        ...
        LOG_DEBUG("Opened streaming file: {}", path);
    } else {
...
bool StreamingFileWriter::writeStreamingChunk(const char* data, size_t size) {
    ...
    if (success) {
        ...
        LOG_ERROR("Failed to write streaming chunk to file: {} ({} bytes)", 
...
bool StreamingFileWriter::closeStreamingFile() {
    ...
    if (success) {
        ...
        stats_.files_written++;
        LOG_DEBUG("Successfully closed streaming file: {}", current_streaming_path_);
    } else {
        LOG_ERROR("Failed to close streaming file: {}", current_streaming_path_);
    }
...
bool StreamingFileWriter::forceCloseStreamingFile() {
    ...
    LOG_WARN("Force closing streaming file: {}", current_streaming_path_);
