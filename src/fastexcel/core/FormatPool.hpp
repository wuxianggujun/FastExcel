...
namespace std {
template<>
struct hash<fastexcel::core::FormatKey> {
...
public:
     ...
     * @brief 格式迭代器
     */
    std::vector<std::unique_ptr<Format>>::const_iterator begin() const { return formats_.begin(); }
    std::vector<std::unique_ptr<Format>>::const_iterator end() const { return formats_.end(); }
