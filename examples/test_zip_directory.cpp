...
int main() {
    // 设置日志级别
    // 初始化FastExcel库
    if (!fastexcel::initialize("logs/zip_directory_test.log", true)) {
        std::cerr << "Failed to initialize FastExcel library" << std::endl;
        return 1;
    }
    
    std::cout << "=== ZIP修复测试 ===" << std::endl;
    
    // 源目录和目标文件
    std::string sourceDir = "simple_test";
    std::string outputFile = "test_fixed_directory.xlsx";
