...
int main() {
    try {
        ...
        if (!source_path.exists()) {
            LOG_ERROR("错误：源文件不存在: {}", source_file);
            return -1;
        }
        ...
        LOG_INFO("4. 开始修改单元格内容和格式");
        
        // 获取第一个工作表
        auto worksheet = workbook->getWorksheet(0);
        if (!worksheet) {
            LOG_ERROR("无法获取第一个工作表");
            return 1;
        }
        ...
        if (verify_workbook) {
            ...
            if (verify_worksheet) {
                ...
                LOG_INFO("   - 验证A1值: \"{}\"", verify_a1.getStringValue());
                LOG_INFO("   - 验证B2值: {}", verify_b2.getNumberValue());
                LOG_INFO("   - 验证C3值: {}", verify_c3.getNumberValue());
                
                LOG_INFO("   - 验证成功：文件编辑功能正常工作");
            }
        } else {
            LOG_ERROR("   - 验证失败：无法重新打开文件");
        }
        
        LOG_INFO("=== 编辑功能测试完成 ===");
        LOG_INFO("编辑后的文件保存在: {}", target_file);
